...
#        run_prover_local.sh <file.lean>:<theorem> ...   for a sorry with no DEMOS entry
set -o pipefail
...
if [[ "$DEMO" == *.lean:* ]]; then
  ENTRY=("$(dirname "$(realpath "$0")")/run_prover_target.py" --file "${DEMO%:*}" --theorem "${DEMO##*:}" --)
else
  ENTRY=(run_prover_bg.py "$DEMO")
fi
cd "$A" && exec "$PY" -u "${ENTRY[@]}" --provider local --local-provider local \
