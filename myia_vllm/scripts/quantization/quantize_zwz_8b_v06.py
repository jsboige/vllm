...
def quantize_model(
    ...
    # Pre-load model with correct class for VL model (llmcompressor 0.6.0 compatibility)
    # This is required because oneshot() internally uses AutoModelForCausalLM which doesn't support VL models
    ...
    recipe = GPTQModifier(
        ...
        ignore=[
            ...
            "visual.",      # Vision transformer (ViT) - CRITICAL: keep original precision
            "merger.",      # Vision-text connector (patch merger) - CRITICAL
        ],
    ...
    logger.info("  Ignored: lm_head, visual.* (ViT), merger.* (connector)")
    ...
    oneshot(
        model=model,  # Pass pre-loaded model instead of string (VL model compatibility)
        tokenizer=processor,  # Use processor as tokenizer
        recipe=recipe,
        output_dir=output_dir,
        dataset=ds,
        max_seq_length=max_seq_length,
        save_compressed=False,  # Avoid packing issue with 4304 intermediate_size
    )
