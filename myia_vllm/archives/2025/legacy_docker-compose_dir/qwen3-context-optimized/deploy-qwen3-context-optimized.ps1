...
if ($args.Count -eq 0) {
    ...
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -All                # Démarrer tous les modèles" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -Model32B           # Démarrer uniquement le modèle 32B" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -Model8B            # Démarrer uniquement le modèle 8B" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -ModelMicro         # Démarrer uniquement le modèle Micro" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -Model8B -ModelMicro # Démarrer les modèles 8B et Micro" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -All -Stop          # Arrêter tous les modèles" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -Status             # Afficher l'état des modèles" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -All -TestContext   # Démarrer tous les modèles et tester la taille de contexte" $Yellow
    Write-ColorMessage "  .\deploy-qwen3-context-optimized.ps1 -All -Verbose       # Démarrer tous les modèles avec plus de détails" $Yellow
    exit
}
