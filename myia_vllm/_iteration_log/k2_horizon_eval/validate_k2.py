#!/usr/bin/env python3
"""Batterie de validation K2-Horizon-36B GPTQ (overlay v0.30.0+PR#56637)
# Adaptee de qwen38_eval/validate38.py : gates spec-dec/vision/preserve/235k retirees (non applicables),
# kwargs reasoning_effort (K2) au lieu de enable_thinking (Qwen). AWQ-INT4 + MTP-3 + fp8 KV (stack stock v0.27.1).

Adaptee de stock_tq_phase2/validate.py. Differences:
  - MODEL = k2-horizon-36b-a4b
...
def t_smoke():
    b, dt = post({"model": MODEL, "max_tokens": 24, "temperature": 0,
                  ...
                  "chat_template_kwargs": {"reasoning_effort": "low"}}, 120)
...
def t_canary():
    ...
    for tag, p in prompts:
        b, dt = post({"model": MODEL, "max_tokens": 300, "temperature": 0.7,
                      ...
                      "chat_template_kwargs": {"reasoning_effort": "low"}}, 240)
...
def t_tools():
    ...
    try:
        b, dt = post({"model": MODEL, "max_tokens": 200, "temperature": 0, "tools": tools,
                      ...
                      "chat_template_kwargs": {"reasoning_effort": "low"}}, 180)
    except Exception as e:
...
def t_single_stream(rounds=3, max_tokens=300):
    ...
    for r in range(rounds):
        b, dt = post({"model": MODEL, "max_tokens": max_tokens, "temperature": 0.7,
                      ...
                      "chat_template_kwargs": {"reasoning_effort": "low"}}, 300)
    ...
    def worker(i):
        try:
            b, dt = post({"model": MODEL, "max_tokens": max_tokens, "temperature": 0.7,
                          ...
                          "chat_template_kwargs": {"reasoning_effort": "low"}}, 600)
            with lock:
