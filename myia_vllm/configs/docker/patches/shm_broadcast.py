...
#
# PATCHED 2026-04-19 for vllm-project/vllm#35104:
# Original implementation used `with _memory_fence_lock:` (a threading.Lock)
# which depends on `_thread.lock.__enter__`'s C method descriptor. Under
# vLLM stacks that JIT-compile or dlopen new C extensions at runtime
# (FlashInfer JIT autotune, Triton autotune, torch.compile, etc.), the
# CPython 3.12 PyCFunction descriptor table can be corrupted, causing
# `SystemError: attempting to create PyCFunction with class but no
# METH_METHOD flag` on subsequent lock acquisitions — killing the worker
# and ultimately the engine.
#
# Replacement uses libc's sched_yield() via ctypes (POSIX) or kernel32's
# SwitchToThread() (Windows). Both provide sequentially consistent memory
# barrier semantics (kernel scheduling boundary = full memory barrier on
# all major architectures), are ~10-20ns, and bypass CPython's descriptor
# layer entirely so they cannot be affected by C extension descriptor
# corruption.
_memory_fence_lock = threading.Lock()  # kept for backward compat (unused)


def _make_memory_barrier():
    """Resolve a libc memory-barrier primitive at import time.

    Falls back to threading.Lock if libc cannot be loaded (which would only
    happen on extremely unusual stacks where the original bug also doesn't
    manifest, since runtime C-extension loads require libc).
    """
    if sys.platform == "win32":
        try:
            kernel32 = ctypes.WinDLL("kernel32", use_last_error=True)
            switch_to_thread = kernel32.SwitchToThread
            switch_to_thread.restype = ctypes.c_int
            switch_to_thread.argtypes = []
            return switch_to_thread
        except (OSError, AttributeError):
            pass
    else:
        # POSIX: prefer sched_yield from libc (full memory barrier on Linux/BSD).
        libc_name = ctypes.util.find_library("c") or "libc.so.6"
        try:
            libc = ctypes.CDLL(libc_name, use_errno=True)
            sched_yield = libc.sched_yield
            sched_yield.restype = ctypes.c_int
            sched_yield.argtypes = []
            return sched_yield
        except (OSError, AttributeError):
            # Try Python's os.sched_yield as a secondary fallback.
...
_memory_barrier = _make_memory_barrier()
...
def memory_fence():
    ...
    Implementation calls libc's sched_yield (POSIX) or kernel32's
    SwitchToThread (Windows) via ctypes. Both are kernel scheduling
    boundaries which act as sequentially consistent memory barriers on
    all major architectures. ~10-20ns overhead, no CPython descriptor
    dependency.
    """
    _memory_barrier()
