...
- **Qwen3.6-35B-A3B MoE + Genesis TurboQuant k8v4** on port 5002 (GPUs 0,1) — promoted from experimental to production baseline 2026-05-17 after the +35h post-deploy soak completed clean
  ...
  - **Batch LOCKED at `--max-num-batched-tokens 4096` / `--max-num-seqs 16`** (NOT the FP8 baseline's 32768/64) — **kernel-level constraint of the Genesis-TQ image, not a first-boot caution**. Ramp to 32768/64 attempted 2026-05-24 → engine boot crash-loop: torch.compile/Dynamo fake-tensor mismatch `mul(FakeTensor(65536,128), FakeTensor(16*s59,128))` at `determine_available_memory()` (65536 = 2×batched-tokens; TQ k8v4 GQA/multi-query kernel P40 vllm#40792 / P67 can't specialize large batch). **Same signature as DFlash Att2.** Reverted to 4096/16, healthy in ~20s, KV restored 2.045M. Consequence: **watchdog flapping is NOT fixable by ramping batch** — tune the watchdog (`--max-time` 30→60, `MAX_FAIL` 3→5) instead. See MEMORY.md `project_genesis_tq_batch_ceiling.md`.
