...
**Current deployment (2026-08-14)**: **Qwen3.6-35B-A3B MoE + TurboQuant K8V4 on STOCK vLLM `v0.27.1`** — GPUs 0,1, TP=2 + EP=2, image `vllm/vllm-openai:v0.27.1` (a plain `docker pull`, **no patch tree**), profile `medium-qwen36-stock-tq.yml`. gpu-util 0.70, KV **1,030,407 tokens** (3.93× the 262K window), 262K context, vision + thinking + `preserve_thinking`. **GPU 2 free** for CoursIA training jobs. The profile runs an **entrypoint HF-cache phantom-mount guard** (2026-08-12) that refuses to start if the WSL HF bind resolved empty, closing the gap that `create_host_path: false` leaves open across restart-policy restarts.
