...
## Current State (2026-06-24: batch unlocked 4096→8192 via P72/P74, 413 guard removed; v2g baseline since 2026-05-17)
...
- **Qwen3.6-35B-A3B MoE + Genesis TurboQuant k8v4** on port 5002 (GPUs 0,1) — promoted from experimental to production baseline 2026-05-17 after the +35h post-deploy soak completed clean
  ...
  - **Batch UNLOCKED to `--max-num-batched-tokens 8192` / `--max-num-seqs 16` (2026-06-24) via Genesis P72+P74.** The 2026-05-24 "hard 4096 kernel ceiling" conclusion was a **misdiagnosis** — 4096 was a forgotten conservative value, not a constraint. The 4096→32768/64 ramp crashed *only because P72 was OFF*: `GENESIS_ENABLE_P72_PROFILE_RUN_CAP=1` + `GENESIS_PROFILE_RUN_CAP_M=4096` caps the `determine_available_memory()` profile shape at M=4096 (dodging the `mul(FakeTensor(65536,128), FakeTensor(16*s59,128))` Dynamo fake-tensor mismatch) while the **runtime** batch runs at 8192. `GENESIS_ENABLE_P74_CHUNK_CLAMP=1` auto-clamps `long_prefill_token_threshold` to 4096 (prefill chunks ≤4096, decode batches up to 8192) — **this is the structural fix for long-agentic-trace-starves-decode**. Genesis's own TQ-k8v4 launch scripts pair exactly these three with batch 8192. Validated 2026-06-24: boot clean **KV 1,969,973 tokens** (7.51×), **80,880-token continuation prefill → 200 OK** (2.7× harder than the 30K prompt that killed 2026-05-06), single decode 108–111 tok/s, **N=16 → 869.7 tok/s aggregate (+5% vs 829)**, no VRAM leak. 32768/64 stays out of scope (untested with P72, no need). Full detail: MEMORY.md `project_genesis_tq_batch_ceiling.md`.
  - **Oversize-request 413 guard REMOVED 2026-06-24** (`VLLM_MAX_REQUEST_BODY_BYTES=0`): the 512 KB HTTP 413 reject turned slow z.ai→qwen fallback overflow into hard-fails → **cluster-wide cascade outage**. Real levers are the P72/P74 batch unlock + claudish-side concurrency cap, NOT body-size rejection. `error_source_capture` middleware stays ON for logging; set the env >0 to re-enable the reject if ever needed.
